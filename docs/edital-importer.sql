...
-- Executar no SQL Editor do Supabase como statement ÚNICO.
-- ============================================================================
...
CREATE POLICY "user_editais_select_own"
  ON public.user_editais FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_editais_insert_own" ON public.user_editais;
CREATE POLICY "user_editais_insert_own"
  ON public.user_editais FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_editais_update_own" ON public.user_editais;
CREATE POLICY "user_editais_update_own"
  ON public.user_editais FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_editais_delete_own" ON public.user_editais;
CREATE POLICY "user_editais_delete_own"
  ON public.user_editais FOR DELETE
  USING (auth.uid() = user_id);
