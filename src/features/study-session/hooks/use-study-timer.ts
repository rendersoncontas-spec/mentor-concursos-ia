...
export function useStudyTimer({ technique, onPhaseChange, onCycleComplete }: UseStudyTimerProps) {
  ...
  const getTechniqueSettings = useCallback((t: StudyTechnique) => {
    switch (t) {
      case "POMODORO_25_5":
        return { study: 25 * 60, shortBreak: 5 * 60, longBreak: 15 * 60, cyclesBeforeLong: 4 }
      case "POMODORO_50_10":
        return { study: 50 * 60, shortBreak: 10 * 60, longBreak: 30 * 60, cyclesBeforeLong: 3 }
      case "DEEP_WORK":
        return { study: 90 * 60, shortBreak: 15 * 60, longBreak: 30 * 60, cyclesBeforeLong: 2 }
      default:
        return null // LIVRE, FLOWTIME, PERSONALIZADO
    }
  }, [])
