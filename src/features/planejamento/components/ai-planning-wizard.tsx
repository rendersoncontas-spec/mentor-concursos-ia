...
export function AiPlanningWizard({ open, onOpenChange, onSuccess }: AiPlanningWizardProps) {
  ...
  useEffect(() => {
    if (open) {
      getDisciplinesForAutocomplete().then((res) => {
  ...
  const handleNext = () => {
    if (step === 4 && selectedDisciplines.length === 0) {
      ...
      } else if (combined.includes("polícia") || combined.includes("policia") || combined.includes("prf")) {
        discNames.push("Direito Penal", "Direito Processual Penal", "Legislação Especial", "Informática")
      } else if (combined.includes("tcu") || combined.includes("tribunal de contas") || combined.includes("controle")) {
        discNames.push("Auditoria Governamental", "Contabilidade Pública", "Controle Externo", "Economia")
      } else if (combined.includes("banco") || combined.includes("inss") || combined.includes("caixa")) {
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-center mt-4">
              <div className="flex p-1 bg-muted rounded-xl gap-1">
                <button onClick={() => setTipoDias("semana")} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${tipoDias === "semana" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>Dias da Semana</button>
                <button onClick={() => setTipoDias("escala")} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${tipoDias === "escala" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>Escala de Trabalho</button>
              </div>
            </div>
            {tipoDias === "semana" ? (
              <div className="flex flex-col items-center">
                <div className="flex flex-wrap gap-3 justify-center mt-4 max-w-sm">
                  {DIAS_SEMANA.map(dia => {
                    ...
                    return (
                      <button key={dia.id} onClick={() => toggleDia(dia.id)} className={`px-6 py-3 rounded-full border-2 text-sm font-bold transition-all ${isSelected ? 'border-[#2563EB] bg-[#2563EB] text-white shadow-md' : 'border-muted bg-card hover:border-[#2563EB]/40 text-foreground'}`}>
                        {dia.label}
                      </button>
                    )
                  })}
              ...
              <div className="flex flex-col items-center mt-4 space-y-4 max-w-sm mx-auto">
                <div className="grid grid-cols-2 gap-3 w-full">
                  {["12x36", "24x72", "24x48", "5x1 (Folga 1)", "6x1 (Folga 1)", "4x2 (Folga 2)", "Outra"].map(esc => (
                    <button key={esc} onClick={() => setEscalaTrabalho(esc)} className={`py-3 rounded-xl border-2 text-xs font-bold transition-all ${escalaTrabalho === esc ? 'border-[#2563EB] bg-[#2563EB]/10 text-[#2563EB]' : 'border-muted bg-card hover:border-[#2563EB]/40 text-muted-foreground'}`}>
                      {esc}
                    </button>
                  ))}
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4 mt-8">
              <div onClick={() => setMetodo("ciclo")} className={`p-5 rounded-2xl border-2 cursor-pointer transition-all flex items-center gap-4 ${metodo === "ciclo" ? 'border-[#2563EB] bg-[#2563EB]/5' : 'border-muted bg-card hover:border-[#2563EB]/40'}`}>
                <div className={`p-3 rounded-xl ${metodo === "ciclo" ? 'bg-[#2563EB] text-white' : 'bg-muted text-foreground'}`}><Loader2 className="w-6 h-6" /></div>
                <div><h3 className="font-bold text-foreground">Ciclo de Estudos</h3><p className="text-xs text-muted-foreground mt-1">Rotação contínua baseada em pesos e dificuldade. Mais flexível.</p></div>
              </div>
              <div onClick={() => setMetodo("agenda")} className={`p-5 rounded-2xl border-2 cursor-pointer transition-all flex items-center gap-4 ${metodo === "agenda" ? 'border-[#2563EB] bg-[#2563EB]/5' : 'border-muted bg-card hover:border-[#2563EB]/40'}`}>
                <div className={`p-3 rounded-xl ${metodo === "agenda" ? 'bg-[#2563EB] text-white' : 'bg-muted text-foreground'}`}><Calendar className="w-6 h-6" /></div>
                <div><h3 className="font-bold text-foreground">Agenda Semanal</h3><p className="text-xs text-muted-foreground mt-1">Disciplinas amarradas a dias específicos da semana.</p></div>
              </div>
            </div>
          </div>
        )
                  ...
                  <div className="absolute left-0 right-0 top-full mt-1.5 bg-popover border border-border rounded-xl shadow-2xl z-50 overflow-hidden max-h-48 overflow-y-auto divide-y divide-border/30 animate-in fade-in slide-in-from-top-1 duration-150">
                    ...
                    {filteredDbDisciplines.length === 0 ? (
                      <div className="p-3 text-xs text-muted-foreground text-center">
                        Nenhuma matéria exata no banco. Pressione <strong>+ Adicionar</strong> para criar!
                      </div>
                    ) : (
      ...
      case 6:
        ...
        return (
          <div className="space-y-4">
            <div className="text-center space-y-1">
              <h2 className="text-2xl font-black text-foreground tracking-tight">Relevância & Pesos</h2>
              <p className="text-xs font-semibold text-muted-foreground">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="lg:col-span-2 space-y-3.5 max-h-[320px] overflow-y-auto pr-2">
                {selectedDisciplines.map((disc) => (
                  <div key={disc} className="rounded-2xl border border-border p-3.5 space-y-3 bg-card shadow-2xs hover:border-[#2563EB]/40 transition-colors">
                    <h4 className="font-extrabold text-xs text-foreground truncate">{disc}</h4>
                    <div className="grid grid-cols-2 gap-4 text-[10px] font-bold">
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-muted-foreground">
                          <span>IMPORTÂNCIA</span>
                          <span className="text-foreground font-black text-xs">{importanceMap[disc] ?? 3}</span>
                        </div>
                        <Slider
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-muted-foreground">
                          ...
                          <span className="text-foreground font-black text-xs">{knowledgeMap[disc] ?? 3}</span>
                        </div>
                        <Slider
      ...
      case 7:
        return (
          <div className="py-12 flex flex-col items-center justify-center text-center space-y-8">
            ...
            <p className="text-xs text-muted-foreground max-w-sm">Seu ciclo e metas personalizadas para <strong className="text-foreground">{concurso}{cargo ? ` (${cargo})` : ""}</strong> já estão ativos.</p>
            <Button onClick={() => {onSuccess(); onOpenChange(false)}} className="bg-[#2563EB] hover:bg-[#1D4ED8] w-full max-w-sm h-12 text-sm font-bold shadow-lg">Abrir Planejamento</Button>
          </div>
        )
      default: return null
    }
          ...
          <div className="flex flex-col h-auto max-h-[85vh]">
            <div className="p-6 pb-2 shrink-0 border-b border-border/40">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-2 text-[#2563EB] font-bold text-xs bg-[#2563EB]/10 px-3 py-1.5 rounded-full">
                    <Sparkles className="w-3.5 h-3.5" /> Mentor IA
                  </div>
              ...
              <div className="flex gap-1.5 px-2 pb-2">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div key={i} className={`h-1.5 flex-1 rounded-full transition-all ${i <= step ? 'bg-[#2563EB]' : 'bg-muted'}`} />
                ))}
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 relative">
              <AnimatePresence mode="wait">
                <motion.div key={step} className={cn("h-full flex flex-col", (step === 5 || step === 6) ? "justify-start" : "justify-center")}>{renderStepContent()}</motion.div>
              </AnimatePresence>
            </div>
            <div className="p-5 border-t bg-muted/10 shrink-0 flex items-center justify-between">
              {step > 1 && (<button onClick={handleBack} className="text-sm font-semibold text-muted-foreground hover:text-foreground flex items-center"><ArrowLeft className="w-4 h-4 mr-1" /> Voltar</button>)}
              <div className="flex-1" />
              {step < 6 ? (
                  <Button onClick={handleNext} className="bg-[#2563EB] text-white hover:bg-[#1D4ED8] font-bold gap-2 min-w-[120px]">Próximo <ArrowRight className="w-4 h-4" /></Button>
              ) : (
                <Button onClick={handleGenerate} className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-bold gap-2 shadow-lg">Gerar Planejamento <Sparkles className="w-4 h-4" /></Button>
              )}
            </div>
          </div>
        ) : (
