...
const SCORE_META: Record<ScoreBand, { label: string; color: string; bg: string }> = {
  ...
  BOM: { label: "Bom", color: "text-primary", bg: "bg-primary/10 border-primary/30" },
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-xl border bg-card p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="space-y-1">
            <p className="text-[13px] text-foreground font-semibold">Resultado</p>
            <h2 className="text-xl font-semibold tracking-tight">{header.name}</h2>
            <div className="flex flex-wrap gap-1.5">
              ...
              {scoreMeta && (
                <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full border", scoreMeta.bg, scoreMeta.color)}>
                  {scoreMeta.label}
                </span>
              )}
            </div>
          ...
          <div className="text-right">
            <span className={cn("text-4xl font-semibold tabular-nums", accuracyColor(header.accuracy))}>
              {header.accuracy === null ? "—" : `${Math.round(header.accuracy)}%`}
            </span>
            <p className="type-label">de acertos</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
          ...
          <StatCard icon={<Minus className="h-4 w-4 text-muted-foreground" />} label="Brancos" value={header.totalBlank} />
            ...
            {byDiscipline.map((d) => (
              <div key={d.disciplineId ?? d.disciplineName} className="rounded-xl border p-3 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs font-semibold truncate">{d.disciplineName}</span>
                ...
                <div className="flex gap-1 h-2 rounded-full overflow-hidden">
                  ...
                  <span className="bg-muted-foreground/40" style={{ width: `${(d.blank / Math.max(1, d.questions)) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
            ...
            {byTopic.map((t, i) => (
              <div key={`${t.topicId ?? "null"}-${i}`} className="flex items-center justify-between gap-3 rounded-lg px-3 py-2 bg-muted/30">
                <div className="min-w-0">
                  <p className="text-xs font-semibold truncate">{t.topicName ?? "Sem tópico"}</p>
                  <p className="text-[10px] text-muted-foreground font-semibold">{t.disciplineName} · {t.questions} questões</p>
                </div>
                <Badge variant={t.accuracy !== null && t.accuracy >= 70 ? "secondary" : "outline"} className="text-[10px] tabular-nums shrink-0">
      ...
      <section className="rounded-xl border bg-card p-5 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-[13px] font-semibold text-foreground flex items-center gap-2">
            <BookOpenCheck className="h-4 w-4" /> Gabarito comentado
          </h3>
          <Badge variant="outline" className="text-[10px] tabular-nums">{questions.length} questões</Badge>
        </div>
            ...
            return (
              <div key={q.questionId} className="rounded-xl border overflow-hidden">
                ...
                >
                  <span
                    className={cn(
                      "shrink-0 h-7 w-7 rounded-lg flex items-center justify-center font-semibold text-xs",
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-xs font-semibold truncate">{q.statement}</span>
                  ...
                  <span className="text-[10px] font-semibold text-muted-foreground shrink-0">ver detalhes</span>
                </button>
                ...
                {open && (
                  <div className="border-t p-4 space-y-4 bg-muted/10">
                    ...
                    <div className="text-xs space-y-1 font-semibold">
                      <p>
                        Gabarito: <span className="font-semibold text-emerald-600">{q.correctAnswer}</span>
                        {q.selectedAnswer && (
                          <>
                            {" · "}Sua resposta:{" "}
                            <span className={cn("font-semibold", correct ? "text-emerald-600" : "text-rose-600")}>{q.selectedAnswer}</span>
                          </>
                        )}
                      </p>
                    ...
                    {q.explanation && (
                      <div className="rounded-lg border border-border bg-muted/40 p-3 text-xs leading-relaxed">
                        <span className="type-label block mb-1">Explicação</span>
                        {q.explanation}
                      </div>
                    )}
      ...
      <section className="rounded-xl border bg-card p-5 space-y-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
          <div className="rounded-xl bg-muted/40 p-3">
            <p className="text-lg font-semibold tabular-nums">{timeStats.totalSeconds > 0 ? formatTimer(timeStats.totalSeconds) : "—"}</p>
            <p className="type-label mt-0.5">Total</p>
          </div>
          <div className="rounded-xl bg-muted/40 p-3">
            <p className="text-lg font-semibold tabular-nums">{timeStats.avgPerQuestionSeconds ?? "—"}s</p>
            <p className="type-label mt-0.5">Por questão</p>
          </div>
          <div className="rounded-xl bg-muted/40 p-3">
            <p className="text-lg font-semibold tabular-nums text-emerald-600">{timeStats.avgPerCorrectSeconds ?? "—"}s</p>
            <p className="type-label mt-0.5">Por acerto</p>
          </div>
          <div className="rounded-xl bg-muted/40 p-3">
            <p className="text-lg font-semibold tabular-nums text-rose-600">{timeStats.avgPerWrongSeconds ?? "—"}s</p>
            <p className="type-label mt-0.5">Por erro</p>
          </div>
        </div>
      </section>

      <div className="flex flex-wrap justify-center gap-3 pb-6">
        <Button onClick={onNewSimulado} className="rounded-xl font-semibold h-10 px-6 bg-primary hover:bg-primary/90">
          Novo Simulado
        </Button>
        <Button variant="outline" onClick={onSeeHistory} className="rounded-xl font-semibold h-9 px-4">
          Ver histórico
        </Button>
      </div>
    </div>
