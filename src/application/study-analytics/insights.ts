  ...
  return ctx.getCache('ai_insights', () => {
    ...
    if (ctx.history.length < 3) {
      return [{
        ...
        description: 'Complete pelo menos 3 sessões de estudo para que o Mentor IA comece a analisar seu padrão de aprendizagem.',
        severity: 'LOW',
        score: 0
      }]
    }
    ...
    if (aggs.consecutiveStreak >= 7) {
      ...
      insights.push({
        ...
        description: 'Sua frequência de estudos oscilou recentemente. Retome com uma sessão curta hoje para religar o cérebro.',
        severity: 'MEDIUM',
        score: 40,
    ...
    if (aggs.averageFocus !== null) {
      if (aggs.averageFocus < 2.5) {
        insights.push({
          ...
          description: 'Sua média de foco nas sessões recentes caiu bastante. Tente deixar o celular em outro cômodo ou aplicar técnica Pomodoro.',
          severity: 'HIGH',
        ...
        insights.push({
          ...
          description: 'Excelente qualidade nas sessões. Você está conseguindo manter distrações sob controle.',
          severity: 'POSITIVE',
          score: Math.round((aggs.averageFocus / 5) * 100)
        })
      }
    ...
    if (interruptRate > 0.3 && aggs.averageEnergy !== null && aggs.averageEnergy <= 2) {
      insights.push({
        ...
        description: 'Você está interrompendo muitas sessões e reportando baixa energia. Seu cérebro precisa de repouso. Considere reduzir a meta.',
        severity: 'CRITICAL',
    ...
    return insights.sort((a, b) => (severityWeight[b.severity] || 0) - (severityWeight[a.severity] || 0))
  })
}
